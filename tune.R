#################################################
### Modified tune function from e1071 package ###
#################################################
...
tune.control <- function(random = FALSE,
                         ...
                         sampling = c("cross", "fix", "bootstrap", "strat"),
...
tune <- function(method, train.x, train.y = NULL, data = list(),
  ...
  ## parameter handling
  if (tunecontrol$sampling == "cross" | tunecontrol$sampling == "strat" )
  ...
  }else if (tunecontrol$sampling == "strat"){
    strat_tune(train.y, tunecontrol$cross)
    ...
    for (sample in 1:length(train.ind)) {
      ...
      for (reps in 1:tunecontrol$nrepeat) {
        ...
        repeat.errors[reps] <- if (!is.null(tunecontrol$error.fun)){
          if(is.character(tunecontrol$error.fun)){
            1-AUC(attr(pred, "probabilities"))
          }else{
